...
public class FavouriteResource {
	...
	@GetMapping
	public ResponseEntity<DtoCollectionResponse<FavouriteDto>> findAll(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size) {
		log.info("*** FavouriteDto List, controller; fetch all favourites *");
		Pageable pageable = PageRequest.of(page, size);
		return ResponseEntity.ok(new DtoCollectionResponse<>(this.favouriteService.findAll(pageable)));
	}
